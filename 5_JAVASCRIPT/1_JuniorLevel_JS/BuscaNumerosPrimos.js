...
function encontrarNumerosPrimos(arr) {
  ...
  let losNumerosAEvaluar = [23, 4, 76, 9, 3];

function misPrimos (array) {
  let guardarNumerosPrimos = [];

  for (let index = 0; index < array.length; index++) {
    let EsUnNumeroPrimo = true;
    ...
    if (EsUnNumeroPrimo) {
      guardarNumerosPrimos.push(array[index]);
    }
  }
  
  return console.log(guardarNumerosPrimos);
  
}

misPrimos(losNumerosAEvaluar);

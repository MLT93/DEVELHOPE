...
function daysInBudget(budget, ratePerHour) {
  var daysWorked = Math.floor(budget / dayRate(ratePerHour));
  return daysWorked;
}
...
function priceWithMonthlyDiscount(ratePerHour, numDays, discount) {
  var totalHours = numDays * HOUR_DAY_WORK;
  var fullMonths = Math.floor(totalHours / (HOUR_DAY_WORK * 22));
  var remainingHours = totalHours % (HOUR_DAY_WORK * 22);
  var fullMonthCost = fullMonths * dayRate(ratePerHour) * 22;
  var remainingCost = remainingHours * ratePerHour;
  var totalCost = fullMonthCost + remainingCost;
  var discountAmount = fullMonthCost * discount;
  var finalCost = Math.ceil(totalCost - discountAmount);
